...
public class SwordController : MonoBehaviour
{
    ...
    void MoveSword()
    {
        ...
        var target = playerToMouse + playerTransform.position;

        var towards = speed * Time.deltaTime * (target - nextPos).normalized;
        var dist = (target - nextPos);

        // Slowdown is the 
        var slowdown = velocity * (1 - math.min(dist.magnitude / slowDownRadius, 1));

        var movement = towards - slowdown;
